...
/* ⚠️ TODO : remplacer par l'URL réelle une fois le Worker Cloudflare de Tenneville créé */
const PROXY_URL = "https://tenneville-push.TON-COMPTE.workers.dev/llm";
...
const MSG_ERREUR = {
  fr: "Désolé, je n'ai pas pu traiter votre demande. Contactez-nous au **+32 84 45 00 40**.",
  nl: "Sorry, uw verzoek kon niet worden verwerkt. Bel ons op **+32 84 45 00 40**.",
  de: "Entschuldigung, Ihre Anfrage konnte nicht bearbeitet werden. Rufen Sie uns an: **+32 84 45 00 40**."
};

const MSG_CONNEXION = {
  fr: "⚠️ Connexion indisponible. Contactez-nous au **+32 84 45 00 40** ou via [notre site](https://www.tenneville.be/).",
  nl: "⚠️ Verbinding niet beschikbaar. Bel **+32 84 45 00 40** of via [onze website](https://www.tenneville.be/).",
  de: "⚠️ Verbindung nicht verfügbar. Rufen Sie uns an: **+32 84 45 00 40** oder [unsere Website](https://www.tenneville.be/)."
};

const MSG_QUOTA = {
  fr: "⚠️ Le service est temporairement saturé. Réessayez dans quelques instants ou appelez-nous au **+32 84 45 00 40**.",
  nl: "⚠️ De service is tijdelijk overbelast. Probeer het later opnieuw of bel **+32 84 45 00 40**.",
  de: "⚠️ Der Dienst ist vorübergehend überlastet. Versuchen Sie es später oder rufen Sie **+32 84 45 00 40** an."
};

const MSG_TIMEOUT = {
  fr: "⚠️ La réponse prend trop de temps. Vérifiez votre connexion ou contactez-nous au **+32 84 45 00 40**.",
  nl: "⚠️ Het antwoord duurt te lang. Controleer uw verbinding of bel **+32 84 45 00 40**.",
  de: "⚠️ Die Antwort dauert zu lange. Überprüfen Sie Ihre Verbindung oder rufen Sie **+32 84 45 00 40** an."
};
  ...
  try {
    ...
    while (attempted.size < 2) {
      ...
      if (response.status === 429) {
        ...
        console.info("[Tenneville] " + provider + " saturé → bascule sur " + next);
        activeProvider = next;
        provider       = next;
        continue;
      }
